...
def test_entregas_muestra_los_tres_pedidos(con_ordenes):
    r = cliente.get("/")
...
def test_orden_por_fecha_mas_reciente_primero(con_ordenes):
    texto = cliente.get("/").text
    assert texto.index("Pedido 00774") < texto.index("Pedido 00770")


def test_sin_proxy_no_hay_entregas_simuladas(db_limpia, monkeypatch):
