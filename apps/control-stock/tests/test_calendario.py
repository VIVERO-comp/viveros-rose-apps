...
def test_ni_retail_ni_crm_quedan_en_los_menus(cliente):
    """Las dos pestañas se BORRARON en la Fase 5 (24/09/2026): no se
    esconden con una bandera, ya no existen. Ni el menú las nombra ni las
    rutas responden."""
    for pagina in (_abrir(cliente).text, cliente.get("/?tab=stock").text,
        ...
        assert "Control" in pagina
    for ruta in ("/retail", "/crm"):
        assert cliente.get(ruta, follow_redirects=False).status_code == 404
    # La piel del calendario dentro de Twenty NO se tocó.
